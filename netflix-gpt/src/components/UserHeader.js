import React from 'react'
import { useState } from 'react';
import {useSelector} from "react-redux"
import { auth } from '../utils/firebase';
import {useNavigate} from "react-router-dom"
import { signOut } from 'firebase/auth';
import {useDispatch} from "react-redux"
import { removeUser } from '../utils/userSlice';
const UserHeader = () => {
    const [menuVisible,setMenuVisible] = useState(false);
    const navigate=useNavigate();
    const dispatch = useDispatch()
    const pullDownHandler=()=>{
        setMenuVisible(!menuVisible)
       
    }
    const signOutHandler=()=>{
        signOut(auth).then(() => {
            dispatch(removeUser())
            navigate("/");
          }).catch((error) => {
            // An error happened.
          });
    }
 
  return (
    <div>
        <div className='flex'>
          <img 
          className='w-12 h-16 pt-4 m-2'
          alt="usericon"
          src="data:image/png;base64,"
          />
          
            <button onClick={pullDownHandler} className='py-4 mt-6 h-8 w-8 bg-black bg-opacity-80 rounded-full border-white'>⬇️
            </button>
             </div>
             {menuVisible && (<div className='bg-black mt-10 absolute bg-opacity-80 '>
              <div className='text-white p-2 m-2 list-none'>
              <li className='cursor-pointer hover:text-semibold bg-red-700 text-white p-2 m-2'>Profile</li>
              <li className='cursor-pointer hover:text-semibold bg-red-700 text-white p-2 m-2' onClick={signOutHandler}>LogOut</li>
              </div>
             </div>)}
        
        
    </div>
  )
}

export default UserHeader